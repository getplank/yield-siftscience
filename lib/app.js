...
//   @option boolean  return_action           : (deprecated) optional (default: false - can be used to get extra params from sift science responses" https://siftscience.com/resources/tutorials/formulas#add-actions)
//   @option boolean  abuse_types             : optional (default: [] - specify an array of sift science products. This parameter restricts the list of score or workflow decision retrieved to the specific products requested." https://siftscience.com/developers/docs/curl/decisions-api/decision-status. Possible values: Array with one or more of - payment_abuse,promo_abuse,content_abuse,account_abuse,legacy)
//   @option boolean  return_score            : optional (default: false - can be used to return score from sift science synchronously" https://siftscience.com/developers/docs/curl/score-api/synchronous-scores)
//   @option boolean  return_workflow_status  : optional (default: false - can be used to return workflow status from sift science synchronously" https://siftscience.com/developers/docs/curl/workflows-api/workflow-decisions)
...
module.exports = function(__global_opts) {
  ...
  // abuse_types must be a string
  if (__global_opts.abuse_types && !(__global_opts.abuse_types instanceof Array))
  ...
  var __sift_request = function(_opts, _callback) {
    ...
    if (__global_opts.abuse_types && __global_opts.abuse_types instanceof Array) {
      ...
      _opts.qs.abuse_types = __global_opts.abuse_types;
      _opts.useQuerystring = true;
    }
  ...
  // @return promise || void
  //
  __sift_object.user_decisions = function(_user_id, _callback) {
    // _user_id must be a String
    if (typeof(_user_id) !== 'string')
      throw new Error('`_user_id` must be a String.');
  ...
  // Public : Get Order Decisions
  //
  // @param string   _order_id  : required
  // @param function _callback : optional (can be used to override promise and make regular callback. ex: function(_err, _response) { ... })
  //
  // @return promise || void
  //
  __sift_object.order_decisions = function(_order_id, _callback) {
    // _order_id must be a String
    if (typeof(_order_id) !== 'string')
      throw new Error('`_order_id` must be a String.');

    var opts = {
      method:  'GET',
      url:     __sift3_url + "accounts/" + __global_opts.account_id + "/orders/" + _order_id + "/decisions",
      json:    true,
      headers: { 'Authorization': "Basic " + __global_opts.api_key }
    };
